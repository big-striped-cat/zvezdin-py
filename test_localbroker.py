...
def broker_event_factory(
    ...
    return BrokerEvent(
        order_id=order_id, type=event_type, created_at=created_at, price=price
    )
...
class TestLocalBroker:
    def test_long_close_by_take_profit(self):
        ...
        broker = LocalBroker(OrderList())
        order_id = 1
        broker.add_order(order_id, order)

        event = broker_event_factory(
            event_type=BrokerEventType.order_close_by_take_profit, price=Decimal(55)
        )
        broker.handle_remote_event(event)
    ...
    def test_long_close_by_stop_loss(self):
        ...
        event = broker_event_factory(
            event_type=BrokerEventType.order_close_by_stop_loss, price=Decimal(35)
        )
        broker.handle_remote_event(event)
    ...
    def test_short_close_by_take_profit(self):
        ...
        event = broker_event_factory(
            event_type=BrokerEventType.order_close_by_take_profit, price=Decimal(35)
        )
    ...
    def test_short_close_by_stop_loss(self):
        ...
        event = broker_event_factory(
            event_type=BrokerEventType.order_close_by_stop_loss, price=Decimal(55)
        )
        broker.handle_remote_event(event)
    ...
    def test_find_orders_for_auto_close(self):
        ...
        trade_open = trade_factory(
            trade_type=TradeType.BUY, price=Decimal(30), created_at=datetime(2022, 1, 1)
        )
        order = order_factory(
            order_type=OrderType.LONG,
            trade_open=trade_open,
        ...
        broker.add_order(1, order)

        now = datetime(2022, 2, 1)
        assert broker.find_orders_for_auto_close(now) == []

        # auto_close is enabled
        trade_open = trade_factory(
            trade_type=TradeType.BUY, price=Decimal(30), created_at=datetime(2022, 3, 1)
        )
        order = order_factory(
            order_type=OrderType.LONG,
            trade_open=trade_open,
        ...
        assert broker.find_orders_for_auto_close(now) == [1]

...
from lib.levels import calc_local_minimums, calc_local_maximums, \
    calc_levels_by_MA_extremums, get_highest_level, get_lowest_level, Level, group_close_points, deduplicate
...
def test_calc_levels_by_MA_extremums_1():
    ...
    _test_calc_levels(calc_levels, klines_csv_path, dt_from, dt_to, level_low_expected, level_high_expected)


def get_klines(kline_data_range: KlineDataRange, dt_from: datetime, dt_to: datetime):
    res = []
    for path in kline_data_range.path_iter():
        klines = read_klines_from_csv(path, skip_header=True, timeframe=timedelta(minutes=5))
        res.extend(k for k in klines if dt_from <= k.open_time <= dt_to)
    return res


def assert_line_in_level(level: Level, line: Decimal, max_width: Optional[Decimal] = None):
...
def test_calc_levels_by_MA_extremums_2():
    ...
    kline_data_range = KlineDataRange(
        'market_data/BTCBUSD-5m-%Y-%m-%d.csv',
        dt_from.date(),
        dt_to.date()
    )

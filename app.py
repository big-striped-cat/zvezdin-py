...
@click.option(
    "--from", "date_from", type=click.DateTime(), required=True, help="date from"
)
...
@cli.command()
@click.option(
    "--ticker", "ticker", type=str, required=True, help="BTCBUSD, ETHUSDT, ..."
)
@click.option(
    "--from", "date_from", type=click.DateTime(), required=True, help="date from"
)
...
def download_klines(
    ticker: str, date_from: datetime, date_to: datetime, output_dir: str
):
    cur_date = date_from
